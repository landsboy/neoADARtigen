...
mer_lenght = 9
counter = 0
list_of_HLA = ["HLA-A0101", "HLA-A0201", "HLA-A2402", "HLA-B0702"," HLA-B3501", "HLA-B1501", "HLA-B5101"]
...
def pre_proccesing(f):
    with open(f"/home/alu/netlandes/MHCpan/results_common/{f}", 'r') as file:
        first_line = file.readline()
        # Open a new file in write mode
        with open(f"/home/alu/netlandes/MHCpan/new_results_common/{f}", 'w') as f_new:
...
def create_results_file(tuples_of_SB, tuples_of_WB, hla):
    ...
    for file in dir_files:
        pre_proccesing(file)
        path = "/home/alu/netlandes/MHCpan/new_results_common/" + file
...
if __name__ == "__main__":
    with open ("final_results_most_common.tsv", 'w') as final_file:
        final_file.write(f"Cancer type:\tMutation:\tBest-ASO:\tRank:\tHLA:\n")
        ...
        for primary_site in list_of_primary_site:
            # First we will check if there is a folder for this type of cancer:
            fp = f"/home/alu/netlandes/MHCpan/most_common/{primary_site}"
            if not os.path.isdir(fp):
                ...
                for file_name in file_list:
                    file_path = os.path.join(fp, file_name)  # Get the full path of the file
                    os.remove(file_path)
            print(primary_site)
            ...
            for mutation_input in list_of_mutation:
                ...
                if unique_ADAR_mut_list:
                    ...
                    delete_files('/home/alu/netlandes/MHCpan/new_results_common')
                    ...
                    if current_best_aso:
                        with open(f"/home/alu/netlandes/MHCpan/most_common/{primary_site}/{mutation_input}", 'w') as f:
                            f.write(f"The best ASO to this mut is: {str(current_best_aso[1])} with rank of {current_best_aso[2]} in {current_best_aso[4]}")
                        if first_type or (current_best_aso[2] < best_ASO[2]):
                            best_ASO = current_best_aso
                            first_type = False
            if best_ASO:
                final_file.write(f"{primary_site}\t{best_ASO[3]}\t{str(best_ASO[1])}\t{best_ASO[2]}\t{best_ASO[4]}\n")